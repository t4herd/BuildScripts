...
sudo apt-get install -y \
		...
		libdc1394-dev libv4l-dev v4l-utils qv4l2 \
		...
		python3-dev python3-numpy python3-py python3-pytest python3-matplotlib \
		curl libcupti-dev libprotobuf-dev protobuf-compiler \
...
mkdir release
cd release/
cmake -D WITH_CUDA=ON -D WITH_CUDNN=ON -D CUDNN_VERSION="9.16" -D CUDA_ARCH_BIN="6.1" -D CUDA_ARCH_PTX="" -D OPENCV_GENERATE_PKGCONFIG=ON -D WITH_GSTREAMER=ON -D WITH_LIBV4L=ON -D CMAKE_CXX_STANDARD=17 -D ENABLE_CXX11=OFF -D ENABLE_FAST_MATH=ON -D CUDA_FAST_MATH=ON -D WITH_CUBLAS=ON -D OPENCV_DNN_CUDA=ON -D BUILD_opencv_dnn=ON -D WITH_V4L=ON -D WITH_QT=OFF -D WITH_OPENGL=ON -D WITH_EIGEN=ON -D WITH_TBB=ON -D WITH_OPENMP=ON -D BUILD_opencv_python3=ON -D BUILD_opencv_python2=OFF -D BUILD_opencv_world=OFF -D EIGEN_INCLUDE_PATH=/usr/include/eigen3  -D WITH_OPENCL=ON -D WITH_1394=OFF -D WITH_GTK=ON -D OPENCV_ENABLE_NONFREE=ON -D OpenBLAS_INCLUDE_DIR=/usr/include/x86_64-linux-gnu/openblas-pthread -D OpenBLAS_LIB=/usr/lib/x86_64-linux-gnu/libopenblas.so -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_EXAMPLES=OFF -D CMAKE_BUILD_TYPE=RELEASE -D OPENCV_EXTRA_MODULES_PATH=../../opencv_contrib-${version}/modules -D CMAKE_INSTALL_PREFIX=/usr/local ..
